...
describe('Navigation', () => {
    ...
    beforeEach(() => {
      wrapper = shallow(<LoginPage />);
       wrap  = mount(<BrowserRouter><LoginPage /></BrowserRouter>);
    });
   ...
   it('On change the value should change', () => {
    wrap.find('input').at(0).simulate('change', { target: { name: 'username', value: 'creole' } });
    expect(wrap.find('input').at(0).prop('value')).toEqual('creole');
  })
  it('On submit fetch should happen', () => {
    wrap.find('form').simulate('submit')
    fetch.mockResponseOnce(JSON.stringify({ user_id: 3, userName:'Carol', surName:'Rein' }))
